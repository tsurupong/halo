...
| D3 | [CLI Specification](d3-cli-spec.md) — 9-command system (run / project init / trigger / stop\|resume / status / doctor / history / watchdog / enable), flag override rules, project init artifacts, doctor inspection, exit code rules, core delegation map | §4.4, §8.2 | 0010 |
| D4 | [Security Design (skeleton)](d4-security-design.md) — OS-level sandbox not adopted (ADR-0024), standard deny set, fine-grained PAT, loop-audit 7 inspections, graph-write control, injection countermeasures, MCP permissions (v1.8-compliant) | §6, §7, §11.1 | 0004, 0011, 0024 |
...
| D9 | [Reliability Design](d9-reliability-design.md) — watchdog supervisor + scheduling/heartbeat, status aggregation, transient-failure requeue/quarantine, graceful shutdown | §3.2, §8.2 | 0013, 0014, 0022, 0023 |
