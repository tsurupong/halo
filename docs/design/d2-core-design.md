...
| L2 | the L1 set + any `minAutonomy: L2` sink |
| L3 | the L2 set + any `minAutonomy: L3` sink, and every sink that declares no `minAutonomy` |

Autonomy is cumulative (L3 ⊇ L2 ⊇ L1). The bundled set currently declares `L1` for both `sink-git-commit` (ADR-0016: a local commit is evidence, not publication) and `sink-progress-log`, so **nothing extra runs above L1 yet**; a `create-pr` sink declaring `L2` (and branching draft/normal on the `AUTONOMY` env within one sink) is deferred past Phase 1. See D1 §1.5.
...
> **Timing of mcp.json generation** (deferred to Phase 4): the `.halo/mcp.json` passed to the executor is to be generated by merging `ports/mcp.d/*.json` (D1 §1.10) once in the heavy stage, with `--strict-mcp-config` making it the sole MCP source; the merge is a deep-merge by the config/discovery module (last-wins, numeric order). **Neither the merge nor the `--mcp-config` flag is implemented yet** — the executor runs with `--strict-mcp-config` and no config, i.e. zero MCP servers, which is the fail-closed direction. Both halves must land together to have any effect.
...
| Port | Unit | Entry |
|---|---|---|
| task-source / context / executor / gate / sink / on-fail | Directory with a `plugin.json` | The `entry` of `plugin.json`, spawned as `node <entry>` (ADR-0018) |
| runtime / trigger | Same, with roles in `aux` | runtime: `entry`=setup, `aux.check` / `aux.test`; trigger: `entry`=fire, `aux.install` / `aux.uninstall` |
...
| Loadability | Valid `plugin.json` + the resolved `entry` exists | Evacuated as `*.disabled`, or a `.disabled` marker file in the directory |

ON/OFF for measuring effect is done by directory operations only (Requirements §3.2 Principle 2 "measure one variable at a time"), and the core / discovery is unchanged.

**Deliberate disable vs. broken configuration are different outcomes.** A `.disabled` marker means "the operator turned this off" and is silent. A candidate that has a `plugin.json` but fails validation — malformed manifest, an `exec` field left over from ≤ v0.2.0, a resolved `entry` that does not exist because `dist/` was not built or the repository moved — is a **configuration defect**. `discoverPort` returns those in `issues` rather than throwing, and the run wiring **refuses to start the loop** while any is present, listing each one.

The reason is that the alternative is worse than a failed launch: dropping a rejected plugin silently means an unattended run can lose `gate-loop-audit` and keep passing iterations with a thinner safety net, and nobody finds out. ADR-0004 rejected "detect and warn, then continue" for exactly this reason ("warnings are read by no one during unattended operation"). Zero enabled candidates for a single port (task-source / executor) travels through the same channel (§2.7).
