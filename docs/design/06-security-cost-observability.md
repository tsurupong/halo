...
| Minimize tool permissions | Limit `--allowedTools` to `mcp__codegraph__*,mcp__knowledge__*,Edit,Write,Bash`, and use `--strict-mcp-config` to ignore the in-project `.mcp.json` and the user global settings (Requirements Document §4.2 executor) | Fixes the visible tool range and prevents inducement of unknown tools |
