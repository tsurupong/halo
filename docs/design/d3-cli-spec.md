...
| 15 | **Ghost claims (`doing/` residue)** | Tasks still claimed (`doing/` residue or an in-progress label) whose claim age exceeds a staleness threshold (default 3600s) (ADR-0025 Decision #4 / Risks). Runs only when the probe is injected — **not wired in the current CLI (`deps.ts` injects no `ghostClaims` probe), so this check is skipped in practice** (wiring it is ADR-0025 follow-up work). Recovery back to `queue/` is the task-source's responsibility; `doctor` only detects and reports | WARN (stale claims found — check whether the owning `launch` died) |
...
- Checks 10/11/14/15 depend on an injected probe and are skipped when it is absent (kept for backward compatibility); everything else always runs. The shipped CLI injects 10/11/14 but not 15 (ghost-claim probe unwired, see above), so a real `halo doctor` reports 15 items.
