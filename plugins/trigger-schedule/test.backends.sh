#!/usr/bin/env bash
# 軽量 backend test: trigger-schedule の install.sh / uninstall.sh が
...
run_script "$WSLBIN" "$TMP/proc.wsl" HALO_HOME=/opt/halo HALO_BIN=/opt/halo/bin/halo -- \
  "$DIR/install.sh" nightly >/dev/null 2>&1
rc=$?
create="$(grep '/Create' "$WSLBIN/schtasks.exe.log" 2>/dev/null)"
if [ $rc -eq 0 ] && \
   [[ "$create" == *"/TN HALO_nightly"* && "$create" == *"/SC DAILY /ST 03:00"* && \
      "$create" == *"/TR wsl.exe -e bash -lc 'HALO_HOME=\"/opt/halo\" HALO_BIN=\"/opt/halo/bin/halo\" $FIRE nightly'"* ]]; then
  pass "(a) WSL stubs: install.sh generates schtasks /Create (DAILY /ST 03:00, env-embedded TR)"
else
...
run_script "$WSLBIN" "$TMP/proc.wsl" HALO_SCHEDULE_TIME=04:30 -- "$DIR/install.sh" nightly >/dev/null 2>&1
create="$(grep '/Create' "$WSLBIN/schtasks.exe.log" 2>/dev/null)"
[[ "$create" == *"/SC DAILY /ST 04:30"* ]] \
  && pass "(a) HALO_SCHEDULE_TIME=04:30 -> /ST 04:30" || flunk "(a) time override create=[$create]"
...
if [ $rc -eq 0 ] && \
   [[ "$state" == *"00 03 * * * HALO_BIN=\"/opt/halo/bin/halo\" $FIRE nightly # HALO:schedule:nightly"* && \
      "$state" == *"/keep/me"* ]]; then
...
run_script "$CRONBIN" "$TMP/proc.plain" -- "$DIR/uninstall.sh" nightly >/dev/null 2>&1
rc=$?
state="$(cat "$STATE" 2>/dev/null)"
if [ $rc -eq 0 ] && [[ "$state" == *"/keep/me"* ]] && [[ "$state" != *"HALO:schedule:nightly"* ]]; then
...
rm -f "$WSLBIN/schtasks.exe.log"
run_script "$WSLBIN" "$TMP/proc.wsl" -- "$DIR/uninstall.sh" nightly >/dev/null 2>&1
rc=$?
log="$(cat "$WSLBIN/schtasks.exe.log" 2>/dev/null)"
[ $rc -eq 0 ] && [[ "$log" == *"/Delete /TN HALO_nightly /F"* ]] \
  && pass "(c) schtasks uninstall calls /Delete" || flunk "(c) schtasks uninstall rc=$rc log=[$log]"

echo
if [ $fail -eq 0 ]; then
  echo "test.backends.sh (trigger-schedule): ALL PASS"
else
  echo "test.backends.sh (trigger-schedule): FAILURES PRESENT"
  exit 1
fi
