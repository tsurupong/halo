...
PATH="$TMP/bin:$PATH" HALO_HOME="$HALO_HOME" HALO_BIN="$INSTALL_BIN" bash "$DIR/install.sh" continuous >/dev/null 2>&1
...
PATH="$TMP/bin:$PATH" bash "$DIR/uninstall.sh" continuous >/dev/null 2>&1
