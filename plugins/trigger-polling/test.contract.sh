#!/usr/bin/env bash
# 軽量 contract test: trigger-polling の fire/install/uninstall を検証する。
...
HALO_HOME="$TMP/missing" bash "$DIR/fire" continuous >/dev/null 2>&1
...
PATH="$TMP/bin:$PATH" HALO_SCHEDULER=schtasks HALO_HOME="$HALO_HOME" HALO_BIN="$INSTALL_BIN" bash "$DIR/install.sh" continuous >/dev/null 2>&1
...
PATH="$TMP/bin:$PATH" HALO_SCHEDULER=schtasks bash "$DIR/uninstall.sh" continuous >/dev/null 2>&1
