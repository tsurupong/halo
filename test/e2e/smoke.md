...
| 層 | 実行 | 課金 | 契機 |
|---|---|---|---|
| ① オフライン dry-run 骨子 | `node scripts/e2e-dry-run.mjs` | なし | いつでも / CI 手動可 |
...
- 使い捨て git フィクスチャリポジトリを生成し、`task-source` / `executor` / `gate` を極小 Node
  モック（entry 契約, ADR-0018）に差し替えて `halo run <profile> --dry-run`（`MAX_ITER=1`）を 1 周実行する。
